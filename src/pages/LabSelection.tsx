...
export function LabSelection({ onLabSelect }: LabSelectionProps) {
  ...
  useEffect(() => {
    // Try to import the service, but fall back to static data if it fails
    import('../services/lumusService')
      .then((module) => {
        console.log('Service imported successfully')
        const labsData = module.labService.getLabs()
        console.log('Got labs from service:', labsData)
        setLabs(labsData)
      })
      .catch((error) => {
        console.error('Error importing service, using static data:', error)
